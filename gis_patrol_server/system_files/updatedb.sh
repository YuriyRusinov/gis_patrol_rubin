...
COMMAND_OK=0
$LINTER_PREFIX/bin/psql -h $DB_HOST -p $DB_PORT -d $BASE -c "select gis_patrol_version()" -t $USER > /dev/null &&
COMMAND_OK=1
...
PREV_VER=`$LINTER_PREFIX/bin/psql -h $DB_HOST -p $DB_PORT -d $BASE -c "select gis_patrol_version()" -t $USER | head -1 | cut -d ' ' -f 5`
...
$LINTER_PREFIX/bin/psql -h $DB_HOST -p $DB_PORT -d $BASE -c "select setgis_patrol_version('DynamicDocs Server version $VERSION', '$VERSION')" -t $USER > /dev/null &&
