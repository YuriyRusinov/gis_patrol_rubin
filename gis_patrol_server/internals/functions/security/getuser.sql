...
DECLARE
    ...
    uname VARCHAR;
BEGIN
    SELECT db_user INTO uname FROM users WHERE id = uid;
    RETURN uname;
END;
$BODY$
LANGUAGE 'plpgsql';
...
DECLARE
    ...
    uid INTEGER;
BEGIN
    SELECT id INTO uid FROM users WHERE db_user = uname;
    RETURN uid;
END;
$BODY$
LANGUAGE 'plpgsql';
...
BEGIN
    ...
    SELECT id INTO uid FROM users WHERE db_user = userName;
    if(uid isnull) then
        return -1;
    end if;
