create or replace function pInsertParameter (int4, varchar, varchar, varchar, varchar, varchar) returns int4 as
$BODY$
declare
...
begin
    ...
    insert into tbl_parameters (id, id_param_type, code, name, title, table_name, column_name, is_system) values (idParameter, idParamType, parameter_code, parameter_name, parameter_title, parameter_table_name, parameter_column_name, false);
    if (not FOUND) then
        return -1;
    end if;
